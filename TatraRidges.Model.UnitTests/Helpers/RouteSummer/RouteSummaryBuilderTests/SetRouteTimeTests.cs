        ...
        public static IEnumerable<object[]> GetSamplesRanges()
        {
            var list = new List<(List<RouteDto?> Routes, TimeSpan time)>()
            {
                ...
                (new List<RouteDto?>(){ RoutesExamples.GetBuilder()
                                                      .SetRouteTime(new TimeSpan(1, 0, 0)).Create() },new TimeSpan(1,0,0)),
                (new List<RouteDto?>(){ RoutesExamples.GetBuilder()
                                                      .SetRouteTime(new TimeSpan(1, 0, 0)).Create(),null},new TimeSpan(1,0,0)),
                (new List<RouteDto?>(){
                                        RoutesExamples.GetBuilder()
                                                      .SetRouteTime(new TimeSpan(2,0,0)).Create(),
                                        RoutesExamples.GetBuilder()
                                                      .Create()
                },new TimeSpan(3,0,0)),
                (new List<RouteDto?>(){
